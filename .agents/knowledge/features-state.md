...
> Dernière mise à jour : 15 août 2026 — PR #300 / Issue #295 : Gestion complète de l'encaissement et paiement des tables avec commandes livrées (`EncaissementModalComponent`), calcul et ventilation détaillée de l'addition (articles, TVA 20%, HT/TTC), paiement unique (Carte, Espèces avec calculateur de rendu de monnaie et jetons rapides, Titres Restaurant, Chèques Vacances, Autre), sélection de pourboires (+5%, +10%, montant libre), remises commerciales (% ou montant fixe), division d'addition (Split égal et split par sélection d'articles attribués aux convives avec suivi en direct), impression ticket thermique 80mm et téléchargement facture PDF A4, libération automatique de la table avec synchronisation STOMP WebSocket temps réel (`/topic/tables`, `/topic/commandes`, `/topic/commandes/statut`), intégration dans `TableDetailModalComponent`, `KanbanServeurComponent` et `DashboardServeurComponent`, 100% CI passée (1257 frontend / 480 backend tests, SonarCloud Quality Gate PASSED, 0 issue).
