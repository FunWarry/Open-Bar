...
export class IngredientFormComponent implements OnInit {
  ...
  constructor(private readonly fb: FormBuilder,private readonly router: Router,private readonly route: ActivatedRoute,private readonly toastCtrl: ToastController,private readonly ingredientService: IngredientService,
  ) {
  ...
  onSubmit(): void {
    ...
    obs$.subscribe({
      next: async () => {
        const toast = await this.toastCtrl.create({ message: this.isEditMode ? 'Ingrédient modifié' : 'Ingrédient créé', duration: 3000, color: 'success' });
        toast.present();
  ...
  onCancel(): void { this.router.navigate(['/ingredients']); }
}
