...
class ShiftPresetServiceTest {
    ...
    @Test
    void getPresetByType_ShouldReturnPreset() {
        when(presetRepository.findByTypeShift(TypeShift.MATIN)).thenReturn(Optional.of(samplePreset));

        ShiftPresetDTO dto = presetService.getPresetByType(TypeShift.MATIN);

        assertThat(dto).isNotNull();
        assertThat(dto.nom()).isEqualTo("Service Matin");
    }

    @Test
    void updatePreset_ShouldSaveAndReturnUpdated() {
        ShiftPresetDTO input = new ShiftPresetDTO(null, TypeShift.MATIN, "Matin Modifié", "07:30", "15:30", 45);
        when(presetRepository.findByTypeShift(TypeShift.MATIN)).thenReturn(Optional.of(samplePreset));
        when(presetRepository.save(any(ShiftPreset.class))).thenAnswer(i -> i.getArgument(0));

        ShiftPresetDTO updated = presetService.updatePreset(TypeShift.MATIN, input);

        assertThat(updated.nom()).isEqualTo("Matin Modifié");
        assertThat(updated.heureDebut()).isEqualTo("07:30");
        assertThat(updated.dureePauseMinutes()).isEqualTo(45);
    }
}
